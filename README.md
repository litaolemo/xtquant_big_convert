...
| **合约/品种** | `get_instrument` / `get_instrument_type` / `get_stock_name` / `get_last_close` / `get_last_volume`（流通股本）/ `get_open_date` / `get_contract_expire_date` / `get_float_caps`（流通股本）/ `get_total_share` / `get_weight_in_index` / `get_svol` / `get_risk_free_rate` / `is_stock_type` / `get_cb_info`；**实测答不了、客户端显式报错**：`get_stock_type`（恒 0）/ `get_turn_over_rate`（恒 None）/ `get_bvol`（恒 0）/ `get_contract_multiplier`（恒 int32 哨兵 2147483647）|
...
>
> **少数方法在实测的这台终端上有名字没答案**（`get_stock_type` 恒 0、`get_turn_over_rate` 恒 None、`get_bvol` 恒 0、`get_contract_multiplier` 恒 `2147483647`）。这几个的封装**显式抛 `NotImplementedError` 并说明实测结果**，不把那个空值递给你 —— 报错看得见，一个恒定的假答案看不见。要自己试的话 `xtdata.call_method("get_bvol", stock=...)` 仍然打得通。
