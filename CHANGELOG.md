...
- **合成周期（1mon/1q/1hy/1y）在部分终端构建上恒返回 0 行**（#237，报告人
  ...
  K 线，`get_local_data` 给得出来；1w / 1d 不受影响，2.1.19.0 六个周期全都正常。
  也就是说不是缺数据，是那一版终端的合成路径。`market_bigqmt.py` 从 0.3.26 到
  现在一行没动，所以升级救不了这类终端。

  现在：主路径连同 #219 重试都空、且周期属于合成周期时，回落到
  `get_local_data`。**只**在这个条件下回落 —— 日线/分钟线的空答案通常是真的，
  多打一次 RPC 不是免费的。

  两件必须说清楚的事：

  - **回落只有 6 列。** 实测 `get_local_data` 只服务
    ...
    0 行。所以对「要全部字段」的请求，回落给的是 OHLCV 六列，缺的列是**不存在**
    而不是 None（读它会抛，不会静默给错值），并在日志里按周期节流报一条
    `[bigqmt_synth_fallback]` WARNING，带上终端构建线索。只要了它服务不了的列的
    调用方，拿回的仍是诚实的空答案。
  - **丢掉 QMT 的补齐行。** `count` 大于实际 K 线数时，`get_local_data` 不是少给
    几行，而是在**头部**补出「四价相同、量额为 0」的假 bar（实测 600519.SH 1y
    count=10：7 行 20171231~20231231 全是 1524.0，那是 2024 年那根的收盘价）。
    把它们当 bar 发出去就是这个仓库最怕的那种静默错误。丢掉之后，回落的答案和
    正常终端主路径的答案**逐行逐值相同** —— 这是实测的，不是推断（见下）。
...
- 在维护者的国金 **2.1.19.0** 实盘终端上（强制 RPC、绕开 FormulaServer）对
  `600519.SH` / `000001.SZ` × `1w/1d/1mon/1q/1hy/1y` 逐格比对：去掉补齐行之后，
  `get_local_data` 与 `get_market_data_ex` 的行**逐值相同**（1mon count=200 时
  主路径 26 行、`get_local_data` 补到 200 行，尾部 26 行完全一致）。所以回落是
  **忠实**的。
- 但 2.1.19.0 上主路径本来就有行，**回落在这台终端是空操作** —— 触发条件从未
  发生。**2.0.8.0 上的实际行为只有单元测试的假终端覆盖**，需要报告人在
  2.0.8.0 上复验。
