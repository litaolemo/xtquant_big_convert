...
> 最后更新：2026-09-22（v0.3.50，#343 四种组合对照）；0.3.28 的 100 方法表保留在下面
...
- **跨线程交接的代价**：后台接收线程从阻塞调用返回后要重新拿 GIL，实测每次约一个 adjust tick（~100ms）。这是所有传输在后台线程模式下慢的原因，`rpc_background_threads: False`（adjust 线程自己非阻塞轮询）就绕开了它。redis **也**受影响——0.3.28 表里 redis + 后台线程 3.4ms 是 adjust 线程 LPOP 抢到请求的那部分，#321 关掉那次抢之后它成了最慢的（下一节）。
...
**这张表里 redis + 后台线程的 3.4ms 已经不成立。** 当时 adjust 线程每拍还会 LPOP
抢请求队列（0.3.28），快的那部分是它抢到的；#321（0.3.47）为了不让重读拖上策略线程把
它关了，之后 redis + 后台线程就是上面四列表里的 407ms。默认值从 0.3.51 起一律 drain。
